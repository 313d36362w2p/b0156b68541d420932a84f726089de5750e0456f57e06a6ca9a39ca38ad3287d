...
def run():
    ...
    preprocessed_dataset = pipeline.get_data_gpds()

    # Split dataset into training and validation sets
    train_size = int(0.3 * len(preprocessed_dataset))
    val_size = len(preprocessed_dataset) - int(0.45 * len(preprocessed_dataset))
    ...
    model.save("utilities/models/inceptionv3.keras", overwrite=True, zipped=True)

    viewer.plot_training_history(history)

    evaluate.far_ffr_test(test_dataset, "utilities/models/inceptionv3.keras")
